...
class ClientAuthenticationAttestation(ClientAuthnMethod):
    ...
    def verify_pop(
        ...
        try:
            ...
            # Some wallet implementations put a placeholder 'kid' in the PoP JWT
            # header (e.g. a fixed "0") instead of leaving it unset, even though
            # the actual verification key always comes from the WIA's 'cnf.jwk'
            # (which has no 'kid'). Align our key's kid with whatever the PoP
            # header declares so cryptojwt's kid-based key filtering doesn't
            # reject the correct key over a meaningless placeholder mismatch.
            _pop_kid = pop_jws.jwt.headers.get("kid")
            if _pop_kid is not None:
                key.kid = _pop_kid
